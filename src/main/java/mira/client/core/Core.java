...
import mira.client.core.manager.client.MacroManager;
...
import static mira.client.features.modules.Module.fullNullCheck;
...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        for (PlayerEntity p : mc.world.getPlayers()) {
            if (p.isDead() || p.getHealth() == 0)
                MiraClient.EVENT_BUS.post(new EventDeath(p));
        }
    ...
    public void drawGps(DrawContext e) {
        if (MiraClient.gps_position != null) {
            float dst = getDistance(MiraClient.gps_position);
            float xOffset = mc.getWindow().getScaledWidth() / 2f;
            float yOffset = mc.getWindow().getScaledHeight() / 2f;
            float yaw = getRotations(new Vec2f(MiraClient.gps_position.getX(), MiraClient.gps_position.getZ())) - mc.player.getYaw();
