...
import mira.client.features.hud.impl.*;
import mira.client.features.modules.Module;
import mira.client.features.modules.combat.*;
import mira.client.features.modules.combat.MaceSwap;
import mira.client.features.modules.combat.ElytraSwapArmor;
import mira.client.features.modules.misc.*;
import mira.client.features.modules.movement.Timer;
import mira.client.features.modules.movement.*;
import mira.client.features.modules.player.*;
import mira.client.features.modules.render.Particles;
import mira.client.features.modules.render.*;
...
public class ModuleManager implements IManager {
    ...
    public static ServerFinder serverFinder = new ServerFinder();
    ...
    public static MiraClientGui miraGui = new MiraClientGui();
    ...
    public void onLoad(String category) {
        try {
            MiraClient.EVENT_BUS.unsubscribe(unHook);
        ...
        modules.forEach(m -> {
            if (m.isEnabled() && (m.getCategory().getName().equalsIgnoreCase(category) || category.equals("none")))
                MiraClient.EVENT_BUS.subscribe(m);
        });
    ...
    public void onRender2D(DrawContext context) {
        ...
        MiraClient.core.onRender2D(context);
    }
    ...
    public void onUnload(String category) {
        modules.forEach(module -> {
            if (module.isEnabled() && (module.getCategory().getName().equalsIgnoreCase(category) || category.equals("none"))) {
                MiraClient.EVENT_BUS.unsubscribe(module);
    ...
    public void registerModule(Module module) {
        ...
        if (module.isEnabled())
            MiraClient.EVENT_BUS.subscribe(module);
    }
