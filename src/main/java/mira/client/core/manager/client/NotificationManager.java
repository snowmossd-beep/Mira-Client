...
import mira.client.features.modules.render.Notifications;
...
public class NotificationManager implements IManager {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "MiraClient");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("MiraClient");
                    tray.add(trayIcon);
                }
    ...
    private void mac(final String message) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + message + "\" with title \"MiraClient\"");
        try {
    ...
    private void linux(final String message) {
        final ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "MiraClient", message);
