package mira.client.core.manager.player;
...
import mira.client.events.impl.EventPostTick;
import mira.client.events.impl.PacketEvent;
import mira.client.events.impl.TotemPopEvent;
import mira.client.features.modules.Module;
import mira.client.features.modules.combat.AntiBot;
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket pac) {
            if (pac.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING) {
                ...
                MiraClient.EVENT_BUS.post(new TotemPopEvent((PlayerEntity) ent, popList.get(ent.getName().getString())));
            }
        }
    }
