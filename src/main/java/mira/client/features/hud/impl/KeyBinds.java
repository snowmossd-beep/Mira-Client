...
public class KeyBinds extends HudElement {
    ...
    public void onRender2D(DrawContext context) {
        ...
        for (Module feature : Managers.MODULE.modules) {
            ...
            if (!Objects.equals(feature.getBind().getBind(), "None") && feature != ModuleManager.clickGui && feature != ModuleManager.miraGui) {
                if (y_offset1 == 0)
                    y_offset1 += 4;
        ...
        if (HudEditor.hudStyle.is(HudEditor.HudStyle.Glowing)) {
            FontRenderers.sf_bold.drawCenteredString(context.getMatrices(), "KeyBinds", getPosX() + hAnimation / 2, getPosY() + 4, HudEditor.hcolor1.getValue().getColorObject());
        } else {
        ...
        if (y_offset1 > 0) {
            if (HudEditor.hudStyle.is(HudEditor.HudStyle.Blurry)) {
                ...
                Render2DEngine.horizontalGradient(context.getMatrices(), getPosX() + 2, getPosY() + 13.7f, getPosX() + 2 + hAnimation / 2f - 2, getPosY() + 14, Render2DEngine.injectAlpha(HudEditor.hcolor1.getValue().getColorObject(), 0), HudEditor.hcolor1.getValue().getColorObject());
                Render2DEngine.horizontalGradient(context.getMatrices(), getPosX() + 2 + hAnimation / 2f - 2, getPosY() + 13.7f, getPosX() + 2 + hAnimation - 4, getPosY() + 14, HudEditor.hcolor1.getValue().getColorObject(), Render2DEngine.injectAlpha(HudEditor.hcolor1.getValue().getColorObject(), 0));
            }
        }
