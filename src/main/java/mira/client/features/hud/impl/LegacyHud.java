...
import mira.client.gui.font.FontRenderer;
...
import mira.client.utility.math.FrameRateCounter;
...
import static mira.client.features.hud.impl.PotionHud.getDuration;
...
public class LegacyHud extends Module {
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (waterMark.getValue())
            drawText(context, "mira v" + MiraClient.VERSION, 2, waterMarkY.getValue());
        ...
        if (speed.getValue()) {
            String str = "Speed " + Formatting.WHITE + MathUtility.round(Managers.PLAYER.currentPlayerSpeed * (bps.getValue() ? 20f : 72f) * MiraClient.TICK_TIMER) + (bps.getValue() ? " b/s" : " km/h");
            drawText(context, str, (width - getStringWidth(str) - 2), renderingUp.getValue() ? (height - 2 - (i += offset)) : (2 + i++ * offset));
        }
        if (chests.getValue()) {
