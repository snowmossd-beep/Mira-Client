package mira.client.features.modules;
...
import mira.client.core.manager.client.CommandManager;
import mira.client.core.manager.client.ModuleManager;
import mira.client.gui.notification.Notification;
import mira.client.features.modules.render.ClientSettings;
import mira.client.features.modules.misc.Windows;
import mira.client.features.modules.misc.UnHook;
...
import static mira.client.MiraClient.LOGGER;
import static mira.client.core.Managers.NOTIFICATION;
...
public abstract class Module {
    ...
    protected void sendPacketSilent(Packet<?> packet) {
        if (mc.getNetworkHandler() == null) return;
        MiraClient.core.silentPackets.add(packet);
        mc.getNetworkHandler().sendPacket(packet);
    }
    ...
    public void enable() {
        ...
        if (isOn()) MiraClient.EVENT_BUS.subscribe(this);
        if (fullNullCheck()) return;

        LogUtils.getLogger().info("[MiraClient] enabled " + this.getName());
    ...
    public void disable() {
        try {
            MiraClient.EVENT_BUS.unsubscribe(this);
        ...
        LOGGER.info("[MiraClient] disabled {}", getName());
