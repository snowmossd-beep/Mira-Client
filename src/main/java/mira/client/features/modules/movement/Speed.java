...
import mira.client.events.impl.*;
import mira.client.injection.accesors.IInteractionManager;
import mira.client.features.modules.Module;
import mira.client.setting.Setting;
import mira.client.utility.interfaces.IEntity;
import mira.client.utility.player.InventoryUtility;
import mira.client.utility.player.MovementUtility;
import mira.client.utility.player.SearchInvResult;

import static mira.client.features.modules.render.ClientSettings.isRu;
import static mira.client.utility.player.MovementUtility.isMoving;
...
public class Speed extends Module {
    ...
    private mira.client.utility.Timer elytraDelay = new mira.client.utility.Timer();
    private mira.client.utility.Timer startDelay = new mira.client.utility.Timer();
    ...
    public void onSync(EventSync e) {
        ...
        if (mode.getValue() == Mode.MatrixJB) {
            ...
            if (MovementUtility.isMoving() && closeToGround && mc.player.fallDistance <= 0) {
                ...
                MiraClient.TICK_TIMER = matrixJBSpeed.getValue();
    ...
    public void modifyVelocity(EventPlayerTravel e) {
        if (mode.getValue() == Mode.GrimEntity && !e.isPre() && MiraClient.core.getSetBackTime() > 1000) {
            for (PlayerEntity ent : Managers.ASYNC.getAsyncPlayers()) {
                if (ent != mc.player && mc.player.squaredDistanceTo(ent) <= 2.25) {
                    float p = mc.world.getBlockState(((IEntity) mc.player).mira_Recode$getVelocityBP()).getBlock().getSlipperiness();
        ...
        if ((mode.is(Mode.GrimEntity2) || mode.is(Mode.GrimCombo)) && !e.isPre() && MiraClient.core.getSetBackTime() > 1000 && MovementUtility.isMoving()) {
            int collisions = 0;
    ...
    public void onTick(EventTick e) {
        ...
        if ((mode.is(Mode.GrimIce) || mode.is(Mode.GrimCombo)) && mc.player.isOnGround()) {
            BlockPos pos = ((IEntity) mc.player).mira_Recode$getVelocityBP();
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            MiraClient.TICK_TIMER = useTimer.getValue() ? 1.088f : 1f;
