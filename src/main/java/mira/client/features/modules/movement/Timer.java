...
public class Timer extends Module {
    ...
    public void onUpdate() {
        switch (mode.getValue()) {
            case Normal -> MiraClient.TICK_TIMER = speed.getValue();
            case Matrix -> {
                ...
                MiraClient.TICK_TIMER = Math.max(speed.getValue(), 1f);
            ...
            case Grim -> {
                if (energy <= 0 || !isKeyPressed(boostKey) || MiraClient.core.getSetBackTime() < 2000) {
                    MiraClient.TICK_TIMER = 1f;
                    return;
                }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        if (mode.is(Mode.Grim)) {
            if (e.getPacket() instanceof CommonPingS2CPacket
                    && MiraClient.core.getSetBackTime() > 2000) {
